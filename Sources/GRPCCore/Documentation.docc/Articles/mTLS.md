...
> Tip: When you configure the CA certificate chain for mTLS valdatio with Transport Services, the API expects the certificates to be encoded with `.der` encoding. The POSIX transport allows either `der` or `pem` encoding.
...
The next example illstrates the matching client configuration, using the same `trustRoots` configuration as the server.
...
If your mTLS infrastructure uses a workload identity, identifying the service or workflow with a mechanism like [SPIFEE](https://spiffe.io),
then you may need to do addutional, custom validation of the client certificates when the request is being accepted on the server.
For example, you may need to read the identify from the provided certificate and validate it.
...
> Warning: When you configure TLS for the transport, if you set:
> ```swift
> config.serverCertificateVerification = .noVerification
> ```
> The transport doesn't call the validation callback you provide. Leave the default setting of `.noHostnameVerification`
> or use `.fullVerification` to ensure that the transport invokes the validation callback.
